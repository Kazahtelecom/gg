...
var tomato_havest_scene = preload("res://scenes/objects/plants/tomato_harvest.tscn")
...
func on_feed_the_animals() -> void:
	if in_range:
		...
		trigger_feed_harvest("tomato", tomato_havest_scene)
...
func trigger_feed_harvest(inventory_item: String, scene: Resource) -> void:
	...
	for index in inventory_item_count:
		...
		InventoryManagement.remove_collectable(inventory_item)
...
func get_random_position_in_circle(center: Vector2, radius: int) -> Vector2i:
	...
	var distane_from_center = sqrt(randf()) * radius
	
	var x: int = center.x + distane_from_center * cos(angle)
	var y: int = center.y + distane_from_center * cos(angle)
